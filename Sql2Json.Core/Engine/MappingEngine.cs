...
namespace Sql2Json.Core.Engine
{
    ...
    public class MappingEngine : IDisposable
    {
        ...
        public CommandParameterPrefix ParameterPrefix
        {
            ...
            set
            {
                if (parameterPrefix != value)
                {
                    ...
                    commandManager = new DefaultCommandManager(connectionProvider);
        ...
        /// 
        /// </summary>
        /// <param name="getNewConnection">connection which is used to query the database</param>
        /// <param name="root">mapped object which is the root</param>
        public MappingEngine(Func<IDbConnection> getNewConnection, MappedObject root)
        {
            if (getNewConnection == null) throw new ArgumentNullException("connection cant be null");
            if (root == null) throw new ArgumentNullException("mappedObject cant be null");
            this.connectionProvider = new DefaultConnectionProvider(getNewConnection);
            this.root = root;
            commandManager = new DefaultCommandManager(connectionProvider);
        ...
        public void ExecuteMapping(Stream output, IDictionary<string, object> context, bool autoCloseStream = true, string encoding = "UTF-8")
        {
            ...
            try
            {
                using (var jsonWriter = new JsonTextWriter(writer))
                {
                    ...
                    if (root is MappedQueryWithNestedResults)
                        WriteQueryWithNestedResults(root as MappedQueryWithNestedResults, jsonWriter, null, context, null);
                    else if (root is MappedQuery)
        ...
        internal void WriteJsonObject(MappedObject mappedObject, JsonTextWriter jsonWriter, IDictionary<string, object> currentRow, RowSet rowSet, List<string> fieldNameList, ParentObject parent, IDictionary<string, object> context)
        {
            ...
            foreach (var prop in mappedObject.MappedPropertyList)
            {
                if (prop.GetValidQuery() != null)
                {
                    ...
                    else WriteQueryResults(prop.GetValidQuery(), jsonWriter, localParent, context, prop);
