...
namespace Sql2Json.Core.Engine
{
    ...
    public class DefaultCommandManager : ICommandManager
    {
        ...
        public DefaultCommandManager(IConnectionProvider connectionProvider)
        {
        ...
        internal void InternalCreateQueryNewCommands(MappedObject mappedObject, CommandParameterPrefix prefix, IDictionary<MappedQuery, ICommandWithParameterSetter> commands)
        {
            if (mappedObject is MappedQuery)
            {
                ...
                commands.Add(mappedQuery, commandSetterFactory.CreateCommand(queryText, connectionProvider, prefix));
            }
        ...
        {
            return string.Format("SELECT * FROM ({0}) S2JORDER ORDER BY S2JORDER.{1} ASC", query, orderColumn);
        }
